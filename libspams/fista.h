...
template <typename T> struct ParamFISTA
{
	ParamFISTA()
	{
		num_threads = 1; max_it = 100; L0 = T(0.1); gamma = T(1.5); tol = T(1e-10);
...
template <typename T> struct ParamReg
{
	ParamReg()
	{
		size_group = 1; lambda2d1 = 0; lambda = 0; lambda3d1 = 0; pos = false; intercept = false; num_cols = 1; graph_st = NULL; tree_st = NULL;
