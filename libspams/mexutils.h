...
template <typename T>
inline mxArray* createMatrix<T>(int m, int n);

/// Create a m x n double matrix
template <double> inline mxArray* createMatrix<double>(int m, int n)
{
...
template <float> inline mxArray* createMatrix<float>(int m, int n)
{
	return mxCreateNumericMatrix(static_cast<mwSize>(m),
